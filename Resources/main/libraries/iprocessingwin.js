...
// win.addEventListener('open',function(){
	//new by Lin
	// var tmpFolder = Ti.Filesystem.applicationDataDirectory;
	var tmpFolder = Ti.Filesystem.createTempDirectory().getNativePath();
	// Ti.API.info('tmpFolder:'+tmpFolder.name());
	Ti.API.info('tmpFolder:'+tmpFolder);
	// var folder =Titanium.Filesystem.getFile(folderDir);
	// if(!folder.exists()){
	    // folder.createDirectory(); 
	// }
	
	var folderDir=tmpFolder+'processingpad';
	Ti.API.info('appFolder:'+folderDir);
	folder =Titanium.Filesystem.getFile(folderDir);
	if(!folder.exists()){
	    folder.createDirectory(); 
	}
	
	folder =Titanium.Filesystem.getFile(folderDir+'/data');
	if(!folder.exists()){
	    folder.createDirectory(); 
	}
	
	
	folder =Titanium.Filesystem.getFile(folderDir+'/libraries');
	if(!folder.exists()){
	    folder.createDirectory(); 
	}
	    
	var libFiles = ['iprocessing.js','iprocessing.lib'];    
	for (var i=0; i<libFiles.length;i++){
		var from_file = Ti.Filesystem.getFile(Ti.Filesystem.resourcesDirectory+'main/libraries/'+libFiles[i]);
		var to_file =  Ti.Filesystem.getFile(tmpFolder+'processingpad/libraries/'+libFiles[i]);
		if (to_file.exists()){
			to_file.deleteFile();
		} 
	...
	from_file = Ti.Filesystem.getFile(Ti.Filesystem.resourcesDirectory+'main/main.html');
	to_file =  Ti.Filesystem.getFile(tmpFolder+'processingpad/processingpad.html');
	if (to_file.exists()){
		to_file.deleteFile();
	} 
	to_file.write( from_file.read() );
	
	from_file = Ti.Filesystem.getFile(Ti.Filesystem.applicationDataDirectory+'tmp/main.pde');
	to_file =  Ti.Filesystem.getFile(tmpFolder+'processingpad/processingpad.pde');
	if (to_file.exists()){
		to_file.deleteFile();
	} 
	to_file.write( from_file.read() );
...
var webviewUrl=tmpFolder+'processingpad/processingpad.html';
Ti.API.info('WebViewUrl:'+webviewUrl);
var webview = Titanium.UI.createWebView({url:webviewUrl});
...
function openPhotos(e) {
	...
	Titanium.Media.openPhotoGallery({
		success: function(event) {
			...
			var f = Titanium.Filesystem.getFile(Titanium.Filesystem.applicationDataDirectory+"processingpad/data/"+filename);
